...
export class TaskService {
  ...
  async update(id: string, updateTaskDto: UpdateTaskDto, userEmail: string, isSuperUser: boolean) {
  ...
  async move(id: string, moveTaskDto: MoveTaskDto, userEmail: string, isSuperUser: boolean) {
    const task = await this.taskModel.findById(id);
    if (!task) {
      throw new NotFoundException('Task not found');
    }
